...
function custom_bg_srcset_shortcode($attr, $content = null)
{
    ...
    if(isset($attr['position']) && !in_array($attr['position'], [null, false, ''])){
        $position = $attr['position'];
    }   
	
	$md_position = "center center";
    if(isset($attr['md_position']) && !in_array($attr['md_position'], [null, false, ''])){
        $md_position = $attr['m_position'];
    } 

    $m_position = "center center";
    if(isset($attr['m_position']) && !in_array($attr['m_position'], [null, false, ''])){
	...
	return '<style>  
		...
		';
                  ...
                  background-position:'.$m_position.' !important;
                  }
		...
		';
                  background-repeat: no-repeat;
                  background-position:'.$md_position.' !important;
                  }
...
function cta_shortcode_shortcode($attr)
{
	...
	// Define array of template IDs that should use custom images
	$custom_template_ids = [41839,62161,62160]; // Add your template IDs here
	
	// Set image names based on template ID
	if (in_array($template_id, $custom_template_ids)) {
		// Custom image names for specified templates
		$cta_content_img_width = "mw-lg-270"; 
		$cta_content_para = "cta_content_para"; 
		$cta_content_blocks = "flex-column flex-lg-row align-items-center";
		$desktop_cta_img = "in-content-cta-fullwidth";
		$mobile_cta_img = "m-in-content-cta-img";
	} else {
		// Default image names for other templates
		$cta_content_img_width = "mw-lg-284"; 
		$cta_content_para = "";  
		$cta_content_blocks = "flex-column"; 
		$desktop_cta_img = "in-content-cta-img"; // Replace with your desired image name
		$mobile_cta_img = "m-in-content-cta-img"; // Replace with your desired mobile image name
	}
	...
	$backgroungImage =
		...
		"/img/in-content-cta/cta-bg.webp";
	$img1x = [
			get_exist_image_url("in-content-cta", "cta-bg"),
			get_exist_image_url("in-content-cta", "cta-bg"),
			get_exist_image_url("in-content-cta", "m-cta-bg"),
		];
		$img2x = [
			get_exist_image_url("in-content-cta", "cta-bg@2x"),
			get_exist_image_url("in-content-cta", "cta-bg@2x"),
			get_exist_image_url("in-content-cta", "m-cta-bg@2x"),
		];

		$img3x = [
			get_exist_image_url("in-content-cta", "cta-bg@3x"),
			get_exist_image_url("in-content-cta", "cta-bg@3x"),
			get_exist_image_url("in-content-cta", "m-cta-bg@3x"),
		];
	...
	if (
		...
		echo '<div class="d-flex flex-column justify-content-center true_black_bg rpx_radius_10 flex-md-row-reverse in-content-cta-section overflow-hidden">
			<div class="col-12 col-md '.$cta_content_img_width .'">
				<img src="' . get_exist_image_url("in-content-cta", $desktop_cta_img) . '" srcset="' . get_exist_image_url("in-content-cta", $desktop_cta_img) . ' 1x, ' . get_exist_image_url("in-content-cta", $desktop_cta_img . "@2x") . ' 2x, ' . get_exist_image_url("in-content-cta", $desktop_cta_img . "@3x") . ' 3x"  width="285" height="336" class="img-fluid w-100 d-none d-md-block desktop-cta-img">
				<img src="' . get_exist_image_url("in-content-cta", $mobile_cta_img) . '" srcset="' . get_exist_image_url("in-content-cta", $mobile_cta_img) . ' 1x, ' . get_exist_image_url("in-content-cta", $mobile_cta_img . "@2x") . ' 2x, ' . get_exist_image_url("in-content-cta", $mobile_cta_img . "@3x") . ' 3x"  width="345" height="154" class="img-fluid d-block d-md-none w-100 mobile-cta-img">
				</div> 
				<div class="col-12 col-md rpx_p_20 rpx_px_xl_40 rpx_py_xl_40 mw-md-319 mx-auto align-self-center">
					<div class="d-flex '.$cta_content_blocks .' rpx_gap_20 text-center text-md-start">
					 <h5 class="mb-0 true_white">'.$heading .'</h5>
					 <p class="true_white mb-0 '.$cta_content_para .'">Let us help you BEE proud of your home—contact us and see what all the buzz is about!</p>
...
function accordion_shortcode($atts, $content = null)
{
	...
	return '<div id="' .
		$id .
		'" class="accordion content-accordion">' .
...
<div class="accordion-item border-1">
                <div  class="accordion-header">
                       <h3 class="collapsed py-3 ps-3 pe-5 mb-0 position-relative d-flex align-items-center true_black--imp" data-bs-toggle="collapse" data-bs-target="#' .
		$id .
...
function bc_read_more_shortcode($attr)
{
	...
	echo '<a class="' .
		$background_color .
		'bc_toggle_btn bc_toggle_btn_closed bc_toggle_content link_text_btn" ' .
		$icon_open .
		" " .
		$icon_close .
		' href="#' .
		...
		$default_open_icon .
		' position-relative" aria-hidden="true"></i></a>';
	$output = ob_get_clean();
	return $output;
}
		...
		if ($query->have_posts()):
			while ($query->have_posts()):
                ...
                <div class="col-lg-3 text-lg-start text-center">
                    <a class="d-block text-lg-start text-center footer_text" href="<?php the_permalink(); ?>"><?php echo $ret[0]; ?></a> 
                </div>
                <?php
			endwhile;
...
function child_theme_gravity_submit_button($button, $form) {
	// Change button class based on form ID
	$buttonClass = in_array($form['id'], [4]) ? 'btn-secondary' : 'btn-primary';
    return "<div class='text-center pt-2'><button class='btn {$buttonClass} w-lg-100 rds_gform_submit' id='gform_submit_button_{$form["id"]}'>" . $form["button"]["text"] . "<i class='icon-circle-chevron-right2'></i></button></div>";
}